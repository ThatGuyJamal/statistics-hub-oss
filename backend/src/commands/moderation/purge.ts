    ...
    public async messageRun(ctx: Message, args: Args) {
        ...
        if (numberArg && !channelArg) {
            try {
                ...
                let finishedMessage = await ctx.channel.send({
                    content: "An error occurred while purging messages. Please make sure the messages are not older than 2 weeks.",
                    allowedMentions: { users: [ctx.author.id], roles: [] },
                });
        ...
        if (numberArg && channelArg) {
            ...
            try {
                ...
                return await ctx.channel.send({
                    content: "An error occurred while purging messages. Please make sure the messages are not older than 2 weeks in the channel.",
                    allowedMentions: { users: [ctx.author.id], roles: [] },
                });
            }
        }
