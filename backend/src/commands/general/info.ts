...
export class UserCommand extends ICommand {
    public async messageRun(ctx: Message) {
        ...
        if (!result) return await reply.edit("An error occurred while fetching the data from the api... Please try again later.").catch(() => {})
                ...
                new BaseEmbed().contextEmbed({
                    ...
                    fields: [
                        ...
                        },
                        {
                            ...
                            value: codeBlock("css", 
                            `
${result.popular.map((cmd) => `[${cmd.name}] = Used ${cmd.count} ${cmd.count === 1 ? "time" : "times"}`).slice(0, 5).join("\n") ?? 0}
                            `),
                            inline: false
                        }
                    ], 
