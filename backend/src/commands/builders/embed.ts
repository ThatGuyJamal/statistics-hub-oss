...
export class UserCommand extends ICommand {
  public async messageRun(ctx: Message) {
    return ctx.reply("Please use `/embed` to create an embed. (Slash command)");
  }
  public override async chatInputRun(...[interaction]: Parameters<ChatInputCommand["chatInputRun"]>) {
    ...
    if (titleArgument) {
      if (titleArgument.length > 256)
        return await interaction.followUp("The title is too long. Must be less than 256 characters.");
      embed.setTitle(titleArgument);
    }

    if (descriptionArgument) {
      if (descriptionArgument.length > 4096)
        return await interaction.followUp("Description is too long. Cant be over 2048 characters.");
      embed.setDescription(descriptionArgument);
    }

    if (colorArgument) {
      if (!colorArgument.match(/^#[0-9A-F]{6}$/i))
        return await interaction.followUp(`Your color must be a hexadecimal color code.`);
    ...
    if (footerArgument) {
      if (footerArgument.length > 2048)
        return await interaction.followUp("The footer is too long. Must be less than 2048 characters.");
      embed.setFooter({
        text: footerArgument,
      ...
      (builder) =>
        builder
          ...
          .addStringOption((builder) =>
            builder.setName("title").setDescription("The title of the embed").setRequired(false)
          )
          .addStringOption((builder) =>
            builder.setName("description").setDescription("The description of the embed").setRequired(false)
          )
          .addStringOption((builder) =>
            builder.setName("url").setDescription("The url of the embed").setRequired(false)
          )
          .addStringOption((builder) =>
            builder.setName("color").setDescription("The color of the embed").setRequired(false)
          )
          .addStringOption((builder) =>
            builder.setName("footer").setDescription("The footer of the embed").setRequired(false)
          )
          .addStringOption((builder) =>
            builder.setName("footer-icon").setDescription("The icon for the footer of the embed.").setRequired(false)
          )
          .addStringOption((builder) =>
            builder.setName("author").setDescription("The author of the embed").setRequired(false)
          )
          .addStringOption((builder) =>
          ...
          .addStringOption((builder) =>
            builder.setName("thumbnail").setDescription("The thumbnail of the embed").setRequired(false)
          )
          .addStringOption((builder) =>
            builder.setName("image").setDescription("The image of the embed").setRequired(false)
          )
          .addStringOption((builder) =>
            builder.setName("content").setDescription("The content with the embed").setRequired(false)
          )
